        ...
        while ((line = br.readLine()) != null) {
            ...
            if (p.length >= 8) {
                try {
                    ...
                    double precio = Double.parseDouble(p[7]);
                    animals.add(new Animal(p[0], p[1], p[2], edad, peso, p[5], p[6], precio));
                } catch (NumberFormatException e) {
                    //System.err.println("⚠️ Error en línea: " + line);
                    // opcional: puedes continuar o registrar el error
                }
            }
        }
    } catch (IOException e) {
...
public static class Animal {
    ...
    // 👇 Añade los getters para usarlos en tus pruebas unitarias
